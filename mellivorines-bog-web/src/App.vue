<template>
  <div id="App-Wrapper" :class="[appWrapperClass, theme]" :style="wrapperStyle">
    <Particles v-if="theme === 'theme-dark'"></Particles>
    <div
        id="App-Container"
  ...
  setup() {
    ...
    const fetchWebsiteConfig = () => {
      api.getWebsiteConfig().then(({ data }) => {
        ...
        appStore.websiteConfig = data.data.websiteConfig
      })
    }
    ...
    return {
      ...
      headerImage: computed(() => {
        return {
          backgroundImage: `url(${commonStore.headerImage}), url('@/assets/default-cover.jpg')`,
